...
$(document).on('click', '#btn-crear', function (event) {
  ...
  var curso = {};
  var request = {};
  var id = decodificarBase64(localStorage.getItem('id'));
...
$(document).on('click', '#btn-crearExamen', function (event) {
  ...
  if ($('#examenesRestantes').text() < 1) {

    Swal.fire({
      title: 'Plan Gratuito',
      text: "Excediste el límite para tu plan. Actualízate a plan Premium para tener exámenes ilimitados.",
      icon: 'warning',
